...
fn main() {
	...
	if args.is_empty() {
		...
		match mode.as_str() {
			...
			"--help"|"-h"|"h" => {
				println!("{}", constants::custom::HELPMSG);
			},
			_ => {
						...
						if a.t {	//constant lookup
							match a.s.matches(' ').count() {
								0 => {	//normal lookup
									...
									if let Some(res) = constants::get_prec(WPREC, a.s) {
								...
								1 => {	//conversion shorthand, everything is like the 0 case but twice
									...
									if let Some(nfrom) = constants::get_prec(WPREC, sfrom.to_string()) {
										if let Some(nto) = constants::get_prec(WPREC, sto.to_string()) {
